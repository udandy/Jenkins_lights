...
def alloff():
    GPIO.output(getcode('red'), GPIO.LOW)
    GPIO.output(getcode('buzzer'), GPIO.LOW)
    GPIO.output(getcode('yellow'), GPIO.HIGH)
    GPIO.output(getcode('green'),GPIO.LOW)
    return
...
def setstatus(status):
    if status != "":
        alloff()
        if status == "SUCCESS":
            GPIO.output(getcode('green'), GPIO.HIGH)

        if status == "UNSTABLE":
            GPIO.output(getcode('yellow'), GPIO.HIGH)

        if status == "FAILURE":
            GPIO.output(getcode('red'), GPIO.HIGH)
...
def blinking():
    if keepalive:
        threading.Timer(5.0, blinking).start()
